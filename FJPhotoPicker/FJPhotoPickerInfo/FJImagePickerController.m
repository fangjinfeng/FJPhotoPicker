...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount delegate:(id<FJImagePickerControllerDelegate>)delegate {
    return [self initWithMaxImagesCount:maxImagesCount columnNumber:4 delegate:delegate pushPhotoPickerVc:YES];
}

- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<FJImagePickerControllerDelegate>)delegate {
    return [self initWithMaxImagesCount:maxImagesCount columnNumber:columnNumber delegate:delegate pushPhotoPickerVc:YES];
}

- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<FJImagePickerControllerDelegate>)delegate pushPhotoPickerVc:(BOOL)pushPhotoPickerVc {
    _pushPhotoPickerVc = pushPhotoPickerVc;
    FJAlbumPickerController *albumPickerVc = [[FJAlbumPickerController alloc] init];
...
- (instancetype)initWithSelectedAssets:(NSMutableArray *)selectedAssets selectedPhotos:(NSMutableArray *)selectedPhotos  index:(NSInteger)index{
    return [self initWithSelectedAssets:selectedAssets selectedPhotos:selectedPhotos originalPhotos:nil index:index];
}

/// This init method just for previewing photos / 用这个初始化方法以预览图片
- (instancetype)initWithSelectedAssets:(NSMutableArray *)selectedAssets selectedPhotos:(NSMutableArray *)selectedPhotos originalPhotos:(NSMutableArray *)originalPhotos index:(NSInteger)index {
    FJPhotoPreviewController *previewVc = [[FJPhotoPreviewController alloc] init];
    self = [super initWithRootViewController:previewVc];
    if (self) {
        self.selectedAssets = [NSMutableArray arrayWithArray:selectedAssets];
        self.allowPickingOriginalPhoto = self.allowPickingOriginalPhoto;
        [self configDefaultSetting];
        previewVc.models = originalPhotos;
...
- (void)viewDidLoad {
    ...
    // 默认的外观，你可以在这个方法后重置
    self.oKButtonTitleColorNormal   = kFJPhotoButtonTitleNormalColor;
    self.oKButtonTitleColorDisabled = kFJPhotoButtonTitleDisabledColor;
...
- (void)configBarButtonItemAppearance {
    ...
    if (iOS9Later) {
        barItem = [UIBarButtonItem appearanceWhenContainedInInstancesOfClasses:@[[FJImagePickerController class]]];
    } else {
        barItem = [UIBarButtonItem appearanceWhenContainedIn:[FJImagePickerController class], nil];
    }
    NSMutableDictionary *textAttrs = [NSMutableDictionary dictionary];
...
- (void)pushPhotoPickerVc {
    ...
    if (!_didPushPhotoPickerVc && _pushPhotoPickerVc) {
        ...
        [[FJImageManager manager] getCameraRollAlbum:self.allowPickingVideo allowPickingImage:self.allowPickingImage completion:^(FJAlbumModel *model) {
    ...
    FJAlbumPickerController *albumPickerVc = (FJAlbumPickerController *)self.visibleViewController;
    if ([albumPickerVc isKindOfClass:[FJAlbumPickerController class]]) {
...
- (void)setPickerDelegate:(id<FJImagePickerControllerDelegate>)pickerDelegate {
    _pickerDelegate = pickerDelegate;
    [FJImageManager manager].pickerDelegate = pickerDelegate;
}
...
- (void)setPhotoPreviewMaxWidth:(CGFloat)photoPreviewMaxWidth {
    ...
    [FJImageManager manager].photoPreviewMaxWidth = _photoPreviewMaxWidth;
}

- (void)setPhotoWidth:(CGFloat)photoWidth {
    _photoWidth = photoWidth;
    [FJImageManager manager].photoWidth = photoWidth;
}
...
- (void)setSelectedAssets:(NSMutableArray *)selectedAssets {
    ...
    for (id asset in selectedAssets) {
        FJAssetModel *model = [FJAssetModel modelWithAsset:asset type:[[FJImageManager manager] getAssetType:asset]];
...
- (void)setSortAscendingByModificationDate:(BOOL)sortAscendingByModificationDate {
    ...
    [FJImageManager manager].sortAscendingByModificationDate = sortAscendingByModificationDate;
}
