...
- (void)layoutSubviews
{
    ...
    [self.btnCamera setTitle:[NSBundle tz_localizedStringForKey:@"ZLPhotoBrowserCameraText"] forState:UIControlStateNormal];
    [self.btnAblum setTitle:[NSBundle tz_localizedStringForKey:@"ZLPhotoBrowserAblumText"] forState:UIControlStateNormal];
    [self.btnCancel setTitle:[NSBundle tz_localizedStringForKey:@"ZLPhotoBrowserCancelText"] forState:UIControlStateNormal];
...
- (void)showAlertWithTitle:(NSString *)title message:(NSString *)message
{
    ...
    UIAlertAction *action = [UIAlertAction actionWithTitle:[NSBundle tz_localizedStringForKey:@"ZLPhotoBrowserOKText"] style:UIAlertActionStyleDefault handler:nil];
...
- (void)cell_btn_Click:(UIButton *)btn
{
    if (_arraySelectPhotos.count >= self.maxSelectCount
        && btn.selected == NO) {
        ShowToastLong([NSBundle tz_localizedStringForKey:@"ZLPhotoBrowserMaxSelectCountText"], self.maxSelectCount);
        return;
    }
    ...
    if (!btn.selected) {
        ...
        if (![[FJPhotoTool sharePhotoTool] judgeAssetisInLocalAblum:asset]) {
            ShowToastLong(@"%@", [NSBundle tz_localizedStringForKey:@"ZLPhotoBrowseriCloudPhotoText"]);
            return;
        }
...
{
    if (self.arraySelectPhotos.count > 0) {
        [self.btnCamera setTitle:[NSString stringWithFormat:@"%@(%ld)", [NSBundle tz_localizedStringForKey:@"ZLPhotoBrowserDoneText"], self.arraySelectPhotos.count] forState:UIControlStateNormal];
            ...
            [[FJPhotoTool sharePhotoTool] saveImageToAblum:image completion:^(BOOL suc, PHAsset *asset) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    if (suc) {
                        ...
                        ShowToastLong(@"%@", [NSBundle tz_localizedStringForKey:@"ZLPhotoBrowserSaveImageErrorText"]);
                    }
                    [hud hide];
                    [strongSelf hide];
                });
