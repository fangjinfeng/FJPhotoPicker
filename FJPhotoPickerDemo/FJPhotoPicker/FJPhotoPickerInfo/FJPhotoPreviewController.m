...
@implementation FJPhotoPreviewController
...
- (void)viewDidLoad {
    ...
    FJImagePickerController *_tzImagePickerVc = (FJImagePickerController *)weakSelf.navigationController;
...
- (void)configCollectionView {
    ...
    [_collectionView registerClass:[FJGifPreviewCell class] forCellWithReuseIdentifier:@"FJGifPreviewCell"];
}
...
- (void)configCropView {
    ...
    if (!_tzImagePickerVc.showSelectBtn && _tzImagePickerVc.allowCrop) {
        ...
        [FJImageCropManager overlayClippingWithView:_cropBgView cropRect:_tzImagePickerVc.cropRect containerView:self.view needCircleCrop:_tzImagePickerVc.needCircleCrop];
...
- (void)refreshNaviBarAndBottomBarState {
    ...
    FJAssetModel *model = _models[_currentIndex];
...
- (void)showPhotoBytes {
    [[FJImageManager manager] getPhotosBytesWithArray:@[_models[_currentIndex]] completion:^(NSString *totalBytes) {
...
- (void)select:(UIButton *)selectButton {
    ...
    if (!selectButton.isSelected) {
        ...
        if (_tzImagePickerVc.selectedModels.count >= _tzImagePickerVc.maxImagesCount) {
            NSString *title = [NSString stringWithFormat:[NSBundle tz_localizedStringForKey:@"ZLPhotoBrowserMaxSelectCountText"], _tzImagePickerVc.maxImagesCount];
            ...
            if (model.type == TZAssetModelMediaTypeVideo && !_tzImagePickerVc.allowPickingMultipleVideo) {
                [_tzImagePickerVc showAlertWithTitle:[NSBundle tz_localizedStringForKey:@"ZLPhotoBrowserHandleVideoAsPhotoWhenMultiStateText"]];
            }
        }
    } else {
        ...
        for (FJAssetModel *model_item in selectedModels) {
            if ([[[FJImageManager manager] getAssetIdentifier:model.asset] isEqualToString:[[FJImageManager manager] getAssetIdentifier:model_item.asset]]) {
                ...
                for (NSInteger i = 0; i < selectedModelsTmp.count; i++) {
                    FJAssetModel *model = selectedModelsTmp[i];
...
- (void)doneButtonClick {
    ...
    if (_progress > 0 && _progress < 1 && (_selectButton.isSelected || !_tzImagePickerVc.selectedModels.count )) {
        _alertView = [_tzImagePickerVc showAlertWithTitle:[NSBundle tz_localizedStringForKey:@"ZLPhotoBrowserSynchronizingPhotosFromiCloudText"]];
        return;
    }
    ...
    if (_tzImagePickerVc.allowCrop) { // 裁剪状态
        ...
        FJPhotoPreviewCell *cell = (FJPhotoPreviewCell *)[_collectionView cellForItemAtIndexPath:indexPath];
        UIImage *cropedImage = [FJImageCropManager cropImageView:cell.previewView.imageView toRect:_tzImagePickerVc.cropRect zoomScale:cell.previewView.scrollView.zoomScale containerView:self.view];
        if (_tzImagePickerVc.needCircleCrop) {
            cropedImage = [FJImageCropManager circularClipImage:cropedImage];
        }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    FJImagePickerController *_tzImagePickerVc = (FJImagePickerController *)self.navigationController;
    FJAssetModel *model = _models[indexPath.row];
    
    FJAssetPreviewCell *cell;
    __weak typeof(self) weakSelf = self;
    if (_tzImagePickerVc.allowPickingMultipleVideo && model.type == TZAssetModelMediaTypeVideo) {
        cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"FJVideoPreviewCell" forIndexPath:indexPath];
        ...
        cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"FJGifPreviewCell" forIndexPath:indexPath];
    } else {
        cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"FJPhotoPreviewCell" forIndexPath:indexPath];
        FJPhotoPreviewCell *photoPreviewCell = (FJPhotoPreviewCell *)cell;
...
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([cell isKindOfClass:[FJPhotoPreviewCell class]]) {
        [(FJPhotoPreviewCell *)cell recoverSubviews];
    }
}
...
- (void)collectionView:(UICollectionView *)collectionView didEndDisplayingCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([cell isKindOfClass:[FJPhotoPreviewCell class]]) {
        [(FJPhotoPreviewCell *)cell recoverSubviews];
    } else if ([cell isKindOfClass:[FJVideoPreviewCell class]]) {
        [(FJVideoPreviewCell *)cell pausePlayerAndShowNaviBar];
    }
}
